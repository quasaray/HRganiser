...
class JobResourceIT {
     ...
     *
     * @see com.quasaray.hrganiser.repository.search.JobSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    @Test
    @Transactional
    void getAllJobsByEmployedIsEqualToSomething() throws Exception {
        // Initialize the database
        jobRepository.saveAndFlush(job);
        Employee employed;
        if (TestUtil.findAll(em, Employee.class).isEmpty()) {
            employed = EmployeeResourceIT.createEntity(em);
            em.persist(employed);
            em.flush();
        } else {
            employed = TestUtil.findAll(em, Employee.class).get(0);
        }
        em.persist(employed);
        em.flush();
        job.addEmployed(employed);
        jobRepository.saveAndFlush(job);
        Long employedId = employed.getId();

        // Get all the jobList where employed equals to employedId
        defaultJobShouldBeFound("employedId.equals=" + employedId);

        // Get all the jobList where employed equals to (employedId + 1)
        defaultJobShouldNotBeFound("employedId.equals=" + (employedId + 1));
    }
